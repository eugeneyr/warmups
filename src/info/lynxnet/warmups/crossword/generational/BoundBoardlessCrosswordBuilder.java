...
*
* */
public class BoundBoardlessCrosswordBuilder {
    private ShapelessWordBucket bucket;
    private Collection<BoundBoardlessCrosswordConfiguration> maxConfigs = new LinkedList<BoundBoardlessCrosswordConfiguration>();
    private long count = 0;
    private long maxCount = 0;
    private TreeConfigHolder history = new TreeConfigHolder();
    private int width;
    private int height;
    ...
    public Collection<BoundBoardlessCrosswordConfiguration> getMaxConfigs() {
        return maxConfigs;
    }

    public BoundBoardlessCrosswordBuilder(ShapelessWordBucket bucket, int width, int height) {
        this.bucket = bucket;
        this.width = width;
        this.height = height;
    }
        ...
        for (String word : bucket.getWords()) {
            ...
            for (int y = 0; y < 2; y++) {
                ...
                for (int x = 0; x <= upperBound; x++) {
                    BoundBoardlessCrosswordConfiguration currentConfig = new BoundBoardlessCrosswordConfiguration(bucket, width, height);
                    if (currentConfig.canBeAdded(x, y, Direction.RIGHT, word)) {
                        ...
                        try {
                            if (!history.alreadyWas(wp, currentConfig)) {
                                ConfigTreeNode root = new ConfigTreeNode(wp, null);
                                history.addNode(root);
                                diveDeeper(root.getWordPosition(), root, currentConfig);
                            } else {
                                repeats++;
                            }
                        } catch (Throwable e) {
    ...
    public void diveDeeper(WordPosition rootWp, ConfigTreeNode parent, BoundBoardlessCrosswordConfiguration config) {
        count++;

        if (count % 10000 == 0) {
            long now = System.currentTimeMillis();
            System.out.printf("*** Iteration: %d. Roots: %d Leaves: %d\n*** Time it took to process 10000 placements: %d sec\n",
                    count + 1, history.getRoots().size(), history.getLeaves().size(), (now - recentTimestamp) / 1000);
            recentTimestamp = now;
        }
                    ...
                    for (int crossIdx = 0; crossIdx < potentCross.length(); crossIdx++) {
                        ...
                        if (cc == c) {
                            ...
                            if (config.canBeAdded(x, y, newDir, newWord)) {
                                ...
                                config.add(wp);
                                try {
                                    if (!history.alreadyWas(rootWp, config)) {
                                        ConfigTreeNode configHistNode = new ConfigTreeNode(wp, parent);
                                        history.addNode(configHistNode);
                                        wentDeeper = true;
                                        diveDeeper(rootWp, configHistNode, config);
                                    } else {
                                        repeats++;
                                    }
                                } finally {
...
//                prettyPrint(config, System.out);
    ...
    static void prettyPrint(BoundBoardlessCrosswordConfiguration config, PrintStream out) {
...
//            }
    ...
    void testX() {
        BoundBoardlessCrosswordConfiguration config = new BoundBoardlessCrosswordConfiguration(bucket, width, height);
...
//        BoundBoardlessCrosswordBuilder builder = new BoundBoardlessCrosswordBuilder(bucket, 13, 13);
        ...
        BoundBoardlessCrosswordBuilder builder = new BoundBoardlessCrosswordBuilder(bucket, 11, 11);

        builder.kickOff();
        for (BoundBoardlessCrosswordConfiguration config : builder.getMaxConfigs()) {
            prettyPrint(config, System.out);
        }
    }
}
