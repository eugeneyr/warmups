...
public class CrosswordInABucket implements Cloneable {
    private int width;
    private int height;
    ...
    public CrosswordInABucket(int width, int height) {
        this.width = width;
        this.height = height;
    }
    ...
    public boolean canBeAdded(int x, int y, Direction dir, String word,
                              ...
                              LinkedHashSet<WordPosition> downCandidatePlaces,
                              LinkedHashSet<WordPosition> rightCandidatePlaces) {
        if (playField == null) {
                            ...
                            if (idx >= 0) {
                                ...
                                if (newY >= 0) {
                                    if (canBeAdded(x + i, newY, Direction.DOWN, unused, downCandidates, rightCandidates,
                                            rightCandidatePlaces, downCandidatePlaces)) {
                                        found = true;
                                        System.out.println("down candidate: " + unused);
                                        downCandidates.add(unused);
                                        WordPosition downWP = new WordPosition(x + i, newY, Direction.DOWN, unused);
                                        downCandidatePlaces.add(downWP);
                                        break;
                                    }
                                }
                            }
            ...
            for (int i = 0; i < word.length(); i++) {
                ...
                if (playField[x][y + i] != null) {
                    ...
                    while (xIdx < width && playField[xIdx] != null && playField[xIdx][y + i] != null) {
                        suffix = suffix + playField[xIdx][y + i];
                        xIdx++;
                    }
                    String wordAcross = prefix + c + suffix;
                    if (wordAcross.length() > 1) {
                            ...
                            if (idx >= 0) {
                                ...
                                if (newX >= 0) {
                                    if (canBeAdded(newX, y + i, Direction.RIGHT, unused, downCandidates, rightCandidates,
                                            downCandidatePlaces, rightCandidatePlaces)) {
                                        found = true;
                                        System.out.println("right cand: " + unused);
                                        rightCandidates.add(unused);
                                        WordPosition rightWP = new WordPosition(newX, y + i, Direction.RIGHT, unused);
                                        rightCandidatePlaces.add(rightWP);
                                        break;
                                    }
                                }
                            }
    ...
    @Override
    protected CrosswordInABucket clone() throws CloneNotSupportedException {
        CrosswordInABucket other = (CrosswordInABucket) super.clone();
